...
- (void)initSSChatCellUserInterface
{
    ...
    [_mBackImgButton addTarget:self
                        action:@selector(buttonPressed:)
              forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)setLayout:(SSChatMessagelLayout *)layout
{
    ...
    [self.mHeaderImgBtn setBackgroundImage:[UIImage imageNamed:@"touxiang2"]
    ...
    if(_layout.message.messageFrom == SSChatMessageFromOther){
        [self.mHeaderImgBtn setBackgroundImage:[UIImage imageNamed:@"touxiang1"]
                                      forState:UIControlStateNormal];
    }
