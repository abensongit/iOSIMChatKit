...
#import "CFCProgressHUD.h"

#define CFC_PROGRESS_HUD_COLOR_RGBA(R, G, B, A) [UIColor colorWithRed:((R)/255.f) green:((G)/255.f) blue:((B)/255.f) alpha:(A)]

@interface CFCProgressHUD() {
  UIWindow *window;
...
@implementation CFCProgressHUD

+ (CFCProgressHUD *)shared
{
  static dispatch_once_t once;
  static CFCProgressHUD *progressHUD;
  
  dispatch_once(&once, ^{ progressHUD = [[CFCProgressHUD alloc] init]; });
...
- (id)init
{
  ...
  self.statusFont       = [UIFont systemFontOfSize:15];
  self.statusColor      = [UIColor whiteColor];
  self.spinnerColor     = [UIColor whiteColor];
  self.hudColor         = CFC_PROGRESS_HUD_COLOR_RGBA(0.11, 0.11, 0.11, 0.5);
  self.backgroundColor  = CFC_PROGRESS_HUD_COLOR_RGBA(0.11, 0.11, 0.11, 0.5);
  NSBundle *bundle      = [NSBundle bundleForClass:[self class]];
  self.imageSuccess     = [UIImage imageNamed:@"ProgressHUD.bundle/hud-success" inBundle:bundle compatibleWithTraitCollection:nil];
  self.imageError       = [UIImage imageNamed:@"ProgressHUD.bundle/hud-fail" inBundle:bundle compatibleWithTraitCollection:nil];
...
- (void)hudCreate:(NSString *)status image:(UIImage *)image spin:(BOOL)spin hide:(BOOL)hide interaction:(BOOL)interaction
{
  if (toolbarHUD == nil)
  {
    ...
    toolbarHUD.barStyle = UIBarStyleBlack;
    toolbarHUD.backgroundColor = self.hudColor;
    toolbarHUD.layer.cornerRadius = 15;
  ...
  if (imageView == nil)
  {
    imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 45, 45)];
  }
